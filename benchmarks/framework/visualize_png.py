...
def generate_speedup_chart(data: dict[str, list], output_path: Path):
    ...
    ax.axhline(y=avg_speedup, color='red', linestyle='--', linewidth=2, label=f'Average: {avg_speedup:.2f}x')
...
def generate_combined_chart(data: dict[str, list], output_path: Path):
    ...
    avg_speedup = sum(data["speedups"]) / len(data["speedups"])
    ax2.axhline(y=avg_speedup, color='red', linestyle='--', linewidth=2, label=f'Avg: {avg_speedup:.2f}x')
    ...
    for bar in bars:
        height = bar.get_height()
        ax2.text(bar.get_x() + bar.get_width()/2., height, f'{height:.1f}x',
                ha='center', va='bottom', fontsize=8)
