...
for dir in examples/example_*/; do
    ...
    if [ -z "$rs_bin" ]; then
        rs_bin=$(find "$dir" -maxdepth 1 -type f -executable 2>/dev/null | while read -r f; do
            file "$f" | grep -q "ELF.*executable" && echo "$f" && break
        done)
    fi
    ...
    echo "#!/bin/bash" > "$tmp_py"
    echo "python3 \"$py_script\" --help >/dev/null 2>&1 || true" >> "$tmp_py"
    chmod +x "$tmp_py"

    echo "#!/bin/bash" > "$tmp_rs"
    echo "\"$rs_bin\" --help >/dev/null 2>&1 || true" >> "$tmp_rs"
    chmod +x "$tmp_rs"
    ...
    if bashrs bench -w 3 -i 10 -m -q -o "$json_out" "$tmp_py" "$tmp_rs" 2>/dev/null; then
        ...
        # Calculate speedup
        speedup=$(jq -r "($py_mean / $rs_mean) * 100 | floor / 100" <<< '{}' 2>/dev/null || echo "0")

        printf "   Python: %.2fms (±%.2f) | Rust: %.2fms (±%.2f) | %.2fx\n" "$py_mean" "$py_std" "$rs_mean" "$rs_std" "$speedup"
